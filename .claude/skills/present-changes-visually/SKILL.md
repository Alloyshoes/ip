...
When proposing a commit message for the reviewed changes, include an imperative subject line and a body explaining what changed and why.
...
`scripts/generate-split-view-diff.py` is the bundled standard-library-only generator (no packages to install). Keep the page self-contained except for the optional syntax-highlighting resource loaded by the page from a CDN.
